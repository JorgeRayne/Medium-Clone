<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        @if ($user->image)
            <div>
                <img src="{{$user->imageUrl()}}" alt="{{$user->name}}" class="rounded-full h-40 w-40">
            </div>
        @endif

        <div>
            <x-input-label for="image" :value="__('Image')" />
            <x-text-input id="image" class="block mt-1 w-full p-1 " type="file" name="image" :value="old('image')"/>
